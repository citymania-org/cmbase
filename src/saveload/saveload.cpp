...
struct ZSTDSaveFilter : SaveFilter {
	 ...
	 * @param compression_level The requested level of compression.
	 */
	ZSTDSaveFilter(std::shared_ptr<SaveFilter> chain, uint8_t compression_level) : SaveFilter(chain)
	{
...
static std::optional<SavePreset> ParseSavePreset(const std::string &str)
{
        ...
        if (slf.init_write != nullptr && format == slf.name) {
            ...
            if (level != Clamp<int>(level, slf.min_compression, slf.max_compression)) {
                ...
                ShowErrorMessage(GetEncodedString(STR_CONFIG_ERROR),
                	GetEncodedString(STR_CONFIG_ERROR_INVALID_SAVEGAME_COMPRESSION_LEVEL, level_str), WarningLevel::Critical);
    ...
    ShowErrorMessage(GetEncodedString(STR_CONFIG_ERROR),
    	GetEncodedString(STR_CONFIG_ERROR_INVALID_SAVEGAME_COMPRESSION_ALGORITHM, str), WarningLevel::Critical);
    return {};
}
...
static SavePreset GetLocalSavePreset()
{
	...
	const citymania::SaveLoadFormat *def = &(*it);
 ...
 */
static SaveLoadResult SaveFileToDisk(bool threaded, citymania::SavePreset preset)
{
	try {
		// byte compression;
		...
		uint32_t hdr[2] = { preset.format->tag, TO_BE32(SAVEGAME_VERSION << 16) };
 ...
 */
static SaveLoadResult DoSave(std::shared_ptr<SaveFilter> writer, bool threaded, citymania::SavePreset preset)
{
	...
	if (!threaded || !StartNewThread(&_save_thread, "ottd:savegame", &SaveFileToDisk, true, std::move(preset))) {
		...
		SaveLoadResult result = SaveFileToDisk(false, preset);
		SaveFileDone();
 ...
 * @return Return the result of the action. #SaveLoadResult::Ok or #SaveLoadResult::Error
 */
SaveLoadResult SaveWithFilter(std::shared_ptr<SaveFilter> writer, bool threaded, citymania::SavePreset preset)
{
	try {
		_sl.action = SLA_SAVE;
 ...
 */
static const citymania::SaveLoadFormat *DetermineSaveLoadFormat(uint32_t tag, uint32_t raw_version)
{
	auto fmt = std::ranges::find(_saveload_formats, tag, &citymania::SaveLoadFormat::tag);
	...
	fmt = std::ranges::find(_saveload_formats, SAVEGAME_TAG_LZO, &citymania::SaveLoadFormat::tag);
	if (fmt == std::end(_saveload_formats)) {
...
static SaveLoadResult DoLoad(std::shared_ptr<LoadFilter> reader, bool load_check)
{
	...
	const citymania::SaveLoadFormat *fmt = DetermineSaveLoadFormat(hdr[0], hdr[1]);
...
SaveLoadResult SaveOrLoad(std::string_view filename, SaveLoadOperation fop, DetailedFileType dft, Subdirectory sb, bool threaded)
{
	...
	try {
		...
		if (fop == SaveLoadOperation::Save) { // SAVE game
			...
			return DoSave(std::make_shared<FileWriter>(std::move(*fh)), threaded, citymania::GetLocalSavePreset());
		}
